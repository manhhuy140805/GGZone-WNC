import { ApiError } from '@/lib/utils/httpClient';
import { buildUrl } from '@/lib/constants/api';
...
class UploadService {
  ...
  async uploadVideo(
    ...
    if (!token) {
      ...
      return {
        success: false,
        message: 'Vui lòng đăng nhập để tải lên video',
        error: 'NOT_AUTHENTICATED'
      };
    }
    ...
    return this.uploadVideoWithRetry(file, folder, token, onProgress);
  }
  ...
  private async uploadVideoWithRetry(
    ...
    try {
      ...
      const url = new URL(buildUrl('/api/upload/video'));
      url.searchParams.append('folder', folder);
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500 && attempt < this.MAX_RETRIES) {
          await this.delay(this.RETRY_DELAY * attempt);
          return this.uploadVideoWithRetry(file, folder, token, onProgress, attempt + 1);
        }

        return {
          success: false,
          message: data.message || 'Tải lên video thất bại',
          error: `HTTP_${response.status}`
        };
      }
      ...
      // BE trả về { videoUrl, thumbnailUrl, duration }, cần map sang { url }
      ...
      return {
        ...
        message: data.message || 'Tải lên video thành công'
      };
    } catch (error) {
      // Retry on network errors
      if (attempt < this.MAX_RETRIES) {
  ...
  private validateVideoFile(file: File): { valid: boolean; error?: string } {
    ...
    if (!allowedTypes.includes(file.type)) {
      return {
        valid: false,
        error: 'Định dạng không hỗ trợ. Cho phép: MP4, WebM, OGG, MOV'
      };
    }
    ...
    if (file.size > maxSize) {
      return {
        valid: false,
        error: `Kích thước video vượt quá 100MB (${this.formatFileSize(file.size)})`
      };
    }
