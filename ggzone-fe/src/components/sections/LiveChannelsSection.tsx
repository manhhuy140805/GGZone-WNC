...
import { LiveChannelCard } from "../cards";
...
export const LiveChannelsSection: React.FC<LiveChannelsSectionProps> = ({
  channels,
}) => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const nextSlide = () => {
    setCurrentIndex((prev) => (prev + 1) % Math.ceil(channels.length / 4));
  };

  const prevSlide = () => {
    setCurrentIndex((prev) =>
      prev === 0 ? Math.ceil(channels.length / 4) - 1 : prev - 1
    );
  };
  ...
  return (
    <section className="py-8">
      ...
      <div className="mt-6 flex justify-center gap-1">
        {Array.from({ length: Math.ceil(channels.length / 4) }).map(
          (_, index) => (
            <div
              key={index}
